    ...
    public function supplierPaymentData(Request $request)
    {
        ...
        if(!empty($payments))
        {
            foreach ($payments as $key => $payment)
            {
                $nestedData['id'] = $payment->id;
                $nestedData['key'] = $key;
                $nestedData['date'] = date(config('date_format'), strtotime($payment->date));
                $nestedData['reference_no'] = $payment->payment_reference;
...
//             DB::raw('p.grand_total as Debit'),
//             DB::raw('0 as Credit'),
//             DB::raw("'Debit' as trans_status")
//         );

//     // ✅ Query 2: Get Payments (Credits)
//     $payments = DB::table('payments as pm')
...
//             's.name as supplier_name',
//             DB::raw('0 as Debit'),
//             DB::raw('pm.amount as Credit'),
//             DB::raw("'Credit' as trans_status")
//         );

//     // ✅ Combine results
//     $ledger = $purchases->unionAll($payments)->get();

//     // ✅ Sort the combined collection by created_at
//     $sorted = collect($ledger)->sortBy(function ($item) use ($orderColumn, $orderDir) {
...
//         $data[] = [
//             'key' => $key + 1 + $start,
//             'date' => date(config('date_format'), strtotime($entry->created_at)),
...
//       $startDate = $request->input('start_date', '2025-05-19');
//     $endDate = $request->input('end_date', '2025-12-31');
//     $supplierId = $request->input('supplier_id', 4);
...
//         x.supplier_id,
//         s.name AS supplier_name,
//         SUM(x.Debit) - SUM(x.Credit) AS Debit,
//         0 AS Credit,
//         'Debit' AS trans_status,
//         'Opening Balance' AS trans_mode
//     FROM (
//         SELECT
//             warehouse_id,
//             supplier_id,
...
//             0 AS Credit
//         FROM purchases
//         WHERE supplier_id = " . $supplierId . " AND created_at < '" . $startDate . "'
...
//             pm.amount AS Credit
//         FROM payments pm
//         INNER JOIN purchases p ON pm.purchase_id = p.id
//         WHERE p.supplier_id = " . $supplierId . " AND pm.created_at < '" . $startDate . "'
...
//             p.supplier_id,
//             0 AS Debit,
//             pr.grand_total AS Credit
//         FROM return_purchases pr
//         INNER JOIN purchases p ON pr.purchase_id = p.id
//         WHERE p.supplier_id = " . $supplierId . " AND pr.created_at < '" . $startDate . "'
//     ) AS x
//     INNER JOIN warehouses w ON w.id = x.warehouse_id
//     INNER JOIN suppliers s ON s.id = x.supplier_id
//     GROUP BY x.warehouse_id, w.name, x.supplier_id, s.name

//     UNION ALL

//     -- Transactions during date range
//     SELECT
//         t.purchase_id,
//         t.created_at,
//         t.warehouse_id,
...
//             0 AS Debit,
//             pm.amount AS Credit,
//             'Credit' AS trans_status,
//             'Payment' AS trans_mode
//         FROM payments pm
//         INNER JOIN purchases p ON pm.purchase_id = p.id
//         WHERE p.supplier_id = " . $supplierId . " AND pm.created_at BETWEEN '" . $startDate . "' AND '" . $endDate . "'
...
//             pr.grand_total AS Credit,
//             'Credit' AS trans_status,
//             'Return' AS trans_mode
//         FROM return_purchases pr
//         INNER JOIN purchases p ON pr.purchase_id = p.id
//         WHERE p.supplier_id = " . $supplierId . " AND pr.created_at BETWEEN '" . $startDate . "' AND '" . $endDate . "'
//     ) AS t
...
//     // $bindings = [
//     //     // For opening balance subquery
//     //     $startDate,
//     //     $supplierId, $startDate,
//     //     $supplierId, $startDate,
//     //     $supplierId, $startDate,

//     //     // For transactions during date range
...
//     // Run the query
//     // $ledger = DB::select($query, $bindings);
//     $ledger =  DB::select(DB::raw($query));
// dd( $query,$ledger);
//     // ✅ Combine results
//     // $ledger = $purchases->unionAll($payments)->get();
...
//         $data[] = [
//             'key' => $key + 1 + $start,
//             'created_at' => date(config('date_format'), strtotime($entry->created_at)),
//             'status' => $entry->trans_mode,
//             'wh_name' => $entry->wh_name,
//             'Debit' => $entry->Debit != 0 ? number_format($entry->Debit, 2) : '',
//             'Credit' => $entry->Credit != 0 ? number_format($entry->Credit, 2) : '',
//             'balance' => number_format($runningBalance, 2),
...
public function supplierLedgerData(Request $request)
{
    ...
    // Get raw date input (possibly in d/m/Y format)
    $startDateRaw = $request->input('start_date', '2025-05-19');
    $endDateRaw = $request->input('end_date', '2025-12-31');
    $supplierId = $request->input('supplier_id', 4);

    // Convert to MySQL Y-m-d format if needed
    try {
        ...
        // If conversion fails, use raw value (assume it's already in Y-m-d)
    ...
    $ledger = DB::select('CALL GetSupplierTransactions(?, ?, ?)', [$startDate, $endDate, $supplierId]);
// dd($ledger);
    // Convert each row to an object
    $ledger = array_map(fn($item) => (object) $item, $ledger);

    // Sort the data
    $sorted = collect($ledger)->sortBy(function ($item) use ($orderColumn) {
        return $item->{$orderColumn};
    }, SORT_REGULAR, $orderDir === 'asc')->values();
    ...
    foreach ($paginated as $key => $entry) {
        ...
        $data[] = [
            'key' => $key + 1 + $start,
            'created_at' => date(config('date_format', 'Y-m-d'), strtotime($entry->created_at)),
            'status' => $entry->trans_mode,
            'reference_no' => $entry->reference_no,
            'wh_name' => $entry->wh_name,
            'Debit' => $entry->Debit != 0 ? number_format($entry->Debit, 2) : '',
...
public function supplierLedgerData1(Request $request)
{
    dd($request->all());
    ...
    // Raw query as a single string without variable setting (Laravel doesn't support user variables easily)
    $query = "
    SELECT
        a.purchase_id,
        a.created_at,
        ...
        a.Credit,
        a.trans_status,
        SUM(a.Debit - a.Credit) OVER (ORDER BY a.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS balance,
        a.trans_mode
    FROM (
        -- Opening Balance
        SELECT
        ...
        FROM (
            ...
            FROM payments pm
            INNER JOIN purchases p ON pm.purchase_id = p.id
            WHERE p.supplier_id = ? AND pm.created_at < ?
            ...
            FROM return_purchases pr
            INNER JOIN purchases p ON pr.purchase_id = p.id
            WHERE p.supplier_id = ? AND pr.created_at < ?
        ) AS x
        INNER JOIN warehouses w ON w.id = x.warehouse_id
        INNER JOIN suppliers s ON s.id = x.supplier_id
        GROUP BY x.warehouse_id, w.name, x.supplier_id, s.name
        ...
        FROM (
            ...
            FROM payments pm
            INNER JOIN purchases p ON pm.purchase_id = p.id
            WHERE p.supplier_id = ? AND pm.created_at BETWEEN ? AND ?
            ...
            FROM return_purchases pr
            INNER JOIN purchases p ON pr.purchase_id = p.id
            WHERE p.supplier_id = ? AND pr.created_at BETWEEN ? AND ?
        ) AS t
        INNER JOIN warehouses w ON w.id = t.warehouse_id
        INNER JOIN suppliers s ON s.id = t.supplier_id
    ) AS a
    ...
    $data = DB::select($query, $bindings);
     ...
     $sorted = collect($data)->sortBy(function ($item) use ($orderColumn, $orderDir) {
        return $item->{$orderColumn};
    }, SORT_REGULAR, $orderDir === 'desc')->values();
    ...
    $paginated = $sorted->slice($start, $limit);
// dd($data);
    // Return results as JSON or pass to view
