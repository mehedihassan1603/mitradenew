...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {*/
            ...
            $data['order_discount'] = floatval($data['order_discount'] ?? 0);
            $data['order_discount_value'] = floatval($data['order_discount_value'] ?? 0);
            ...
            // }
            if (is_array($data['added_paid_amount'])) {
                $data['paid_amount'] = array_sum($data['added_paid_amount']);
            }
            ...
            $discount_percent = $data['discount-p'];
            ...
            foreach ($product_id as $i => $id) {
                ...
                $product_sale['sale_unit_id'] = $sale_unit_id;
                 ...
                 $product_sale['unit_price'] = $unit_price[$i];
            $product_sale['unit_discount'] = $unit_discount[$i];
            $product_sale['discount_percent'] = $discount_percent[$i] ;
            ...
            $product_sale['product_model'] = $product_model[$i];
            ...
            if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && !empty($data['added_paid_amount']))) {

    foreach ($data['added_paid_by_id'] as $key => $value) {

        if ($data['added_paid_amount'][$key] > 0) {
            ...
            switch ($value) {
                ...
                case 7: $paying_method = 'Points'; $lims_payment_data->used_points = $data['used_points']; break;
                case 8: $paying_method = 'Pesapal'; break;
                case 9: $paying_method = 'Moneipoint'; break;
                default: $paying_method = $value;
            }

            // Optional: cash register
            if (isset($cash_register_data)) {
            ...
            $data['payment_reference'] = 'spr-'.date("Ymd").'-'.date("his").'-'.$key;
            $lims_payment_data->payment_reference = $data['payment_reference'];

            // Amounts
            $lims_payment_data->amount = $data['added_paid_amount'][$key];
            $lims_payment_data->change = $data['added_paying_amount'][$key] - $data['added_paid_amount'][$key];

            // Method, note, receiver
            $lims_payment_data->paying_method = $paying_method;
            $lims_payment_data->payment_note = $data['added_payment_note'][$key] ?? null;
            $lims_payment_data->payment_receiver = $data['added_payment_receiver'][$key] ?? null;
...
// dd( 'dataaaaaaaaaaaa',$data);
            ...
            elseif ($paying_method == 'Gift Card') {
                ...
                $lims_gift_card_data->expense += $data['added_paid_amount'][$key];
            ...
            elseif ($paying_method == 'Deposit') {
                $lims_customer_data->expense += $data['added_paid_amount'][$key];
                $lims_customer_data->save();
            }
            ...
            elseif ($paying_method == 'Pesapal') {
                $redirectUrl = $this->submitOrderRequest($lims_customer_data, $data['added_paid_amount'][$key]);
                $lims_customer_data->save();
            ...
            elseif ($paying_method == 'Moneipoint') {
                $moneipointSaleData = [
                    'amount' => $lims_sale_data->paid_amount,
                    'reference_no' => $lims_sale_data->reference_no,
                ];
                $this->moneipoint($moneipointSaleData);
                $lims_customer_data->save();
            }
        }
    }
...
// dd($lims_product_with_variant_warehouse_data);
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if(in_array('restaurant', explode(',',$general_setting->modules))) {
         ...
         if ($lims_product_data->product_model) {
            $productModel = ProductModel::find($lims_product_data->product_model);
            if ($productModel) {
                $product[] = $productModel->name; // assuming 'name' is the column for the model name
            } else {
                $product[] = null; // or handle missing model
            }
        }
    ...
    public function productSaleData($id)
    {
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            $product_sale[11][$key] = $product->attribute1;
            $product_sale[12][$key] = $product->attribute2;
            $product_sale[13][$key] = $product->attribute3;
            $product_sale[14][$key] = $product->attribute4;
            $product_sale[15][$key] = $product->attribute5;
            $product_sale[16][$key] = $product->attribute6;
            $product_sale[17][$key] = $product_sale_data->unit_price;
            $product_sale[18][$key] = $product_sale_data->net_unit_price;
            $product_sale[19][$key] = $product_sale_data->unit_discount;
            $product_sale[20][$key] = $product_sale_data->product_model;
        }
        return $product_sale;
    }
