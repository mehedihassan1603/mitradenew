...
class QuotationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Now save the data or handle it accordingly
        // dd($data['note']);
        //return dd($data);
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_quotation['unit_price'] = $unit_price[$i];
            $product_quotation['unit_discount'] = $unit_discount[$i];
            $product_quotation['discount_percent'] = $discount_percent[$i] ;
            ...
            $product_quotation['product_model'] = $product_model[$i];
    ...
    public function sendWhatsApp(Request $request)
    {
        ...
        // Get status text
        $message = "ðŸ›’ Quation Details\n";
        $message .= "ðŸ“§ Email: " . $lims_customer_data->email . "\n";
        $message .= "ðŸ“Œ Reference No: " . $lims_sale_data->reference_no . "\n";
        $message .= "ðŸ“Š Total Qty: " . $lims_sale_data->total_qty . "\n";
        $message .= "ðŸ’° Total Price: " . number_format($lims_sale_data->total_price, 2) . "\n";
        $message .= "ðŸ›ƒ Order Tax: " . $lims_sale_data->order_tax . "\n";
        $message .= "ðŸŽ Order Discount: " . $lims_sale_data->order_discount . "\n";
        $message .= "ðŸšš Shipping Cost: " . $lims_sale_data->shipping_cost . "\n";
        $message .= "ðŸ’µ Grand Total: " . number_format($lims_sale_data->grand_total, 2) . "\n";
        $message .= "ðŸ™ Please let us know as soon as possible!";
...
// dd($lims_product_data);
    ...
    public function productQuotationData($id)
    {
        ...
        foreach ($lims_product_quotation_data as $key => $product_quotation_data) {
            ...
            $product_quotation[8][$key] = $product->attribute1;
            $product_quotation[9][$key] = $product->attribute2;
            $product_quotation[10][$key] = $product->attribute3;
            $product_quotation[11][$key] = $product->attribute4;
            $product_quotation[12][$key] = $product->attribute5;
            $product_quotation[13][$key] = $product->attribute6;
            $product_quotation[14][$key] = $product_quotation_data->unit_price;
            $product_quotation[15][$key] = $product_quotation_data->net_unit_price;
            $product_quotation[16][$key] = $product_quotation_data->unit_discount;
            $product_quotation[17][$key] = $product_quotation_data->product_model;
        }
