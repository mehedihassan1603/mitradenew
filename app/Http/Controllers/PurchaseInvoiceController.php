...
//    }
    ...
    public function store(Request $request)
    {
        ...
        // Save multiple attachments if exists
        if ($request->hasFile('attachments')) {
            foreach ($request->file('attachments') as $file) {
                $fileName = uniqid('file-', true) . '.' . $file->getClientOriginalExtension();
                $file->move(public_path('images/purchase_invoices'), $fileName);
                PurchaseInvoiceAttachment::create([
                    'purchase_invoice_id' => $invoice->id,
                    'file_path'           => 'images/purchase_invoices/' . $fileName,
...

{
    ...
    $invoice = PurchaseInvoice::with([
        ...
        'order.exporter',
        'attachments',
        'items.product.hs', // 👈 use invoice items instead of order items
    ])->findOrFail($id);

    $lcs = LC::all();
    $allcharges = AllChargesList::all();

    return view('backend.purchase_invoice.received', compact('invoice', 'lcs', 'allcharges'));
}
