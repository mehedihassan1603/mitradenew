                ...
                <tbody>
                    @forelse($receivedList as $key => $rec)
                        <tr>
                            ...
                            <td>{{ $rec->bl_number ?? '-' }}</td>
                            <td>{{ $rec->invoice->order->supplier->name ?? '-' }}</td>
                            <td>{{ $rec->invoice->order->exporter->name ?? '-' }}</td>
                            <td>{{ $rec->received_notes ?? '-' }}</td>
                            <td>
                                @if($rec->status == 0)
                                    <span class="badge bg-warning">Pending</span>
                            ...
                            <td>{{ $rec->created_at->format('d M, Y') }}</td>
                            <td>
                                <a href="{{ route('purchase.invoice.received.show', $rec->id) }}" class="btn btn-info btn-sm">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="text-center">No Purchase Received yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
