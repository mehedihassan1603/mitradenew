    ...
    <div class="row">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title mb-4">Update Purchase Requisition</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('purchase-requisition.update', $purchaseRequisition->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" name="date" id="date" class="form-control" value="{{ $purchaseRequisition->date }}" />
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" name="ref_id" id="ref_id" class="form-control" value="{{ $purchaseRequisition->ref_id }}" readonly />
                        ...
                        <select class="form-control" multiple name="product_name[]" id="product_name">
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" data-code="{{ $product->code }}"
                                    {{ in_array($product->id, $purchaseRequisition->product_id ?? []) ? 'selected' : '' }}>
                                    {{ $product->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <table class="table table-bordered mt-4" id="productTable">
                        ...
                        @if(!empty($purchaseRequisition->product_id))
                            @foreach($purchaseRequisition->product_id as $pid)
                                @php
                                    $product = $products->where('id', $pid)->first();
                                    $quantity = $purchaseRequisition->quantities[$pid] ?? 1;
                                @endphp
                                <tr id="product-row-{{ $pid }}">
                                    <td>{{ $product->name }}</td>
                                    <td>{{ $product->code }}</td>
                                    <td>
                                        <input type="number" name="quantities[{{ $pid }}]" value="{{ $quantity }}"
                                               class="form-control quantity-input" min="1" data-product-id="{{ $pid }}">
                                    </td>
                                </tr>
                            @endforeach
                        @endif
                    ...
                    <button type="submit" class="btn btn-primary w-md mt-4">Update Purchase Requisition</button>
                </form>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        productQuantities["{{ $pid }}"] = {{ $purchaseRequisition->quantities[$pid] ?? 1 }};
        @endforeach
...
{{--                        <select class="form-control" multiple name="product_name[]" id="product_name">--}}
{{--                            @foreach($products as $product)--}}
{{--                                <option value="{{ $product->id }}" data-code="{{ $product->code }}" {{ $product->name == $purchaseRequisition->$product_id ? 'selected' : '' }}>--}}
{{--                                    {{ $product->name }}--}}
{{--                                </option>--}}
{{--                            @endforeach--}}
{{--                        </select>--}}
