...
<section class="forms">
    ...
    <ul class="nav nav-tabs ml-4 mt-3" role="tablist">
      ...
      <li class="nav-item">
        <a class="nav-link" href="#ledger"  role="tab" data-toggle="tab">{{trans('Ledger Report')}}</a>
      </li>
    </ul>
    <div class="tab-content">
        ...
        <div role="tabpanel" class="tab-pane fade" id="ledger">
            <div class="table-responsive mb-4">
                <table id="ledger-table" class="table table-hover" style="width: 100%">
                    <thead>
                        <tr>
                            <th class="not-exported-payment"></th>
                            <th>{{trans('file.Date')}}</th>
...
<script type="text/javascript">
    ...
    function datatable_sum_payment(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            $( dt_selector.column( 5 ).footer() ).html(dt_selector.cells( rows, 5, { page: 'current' } ).data().sum().toFixed({{$general_setting->decimal}}));
$( dt_selector.column( 6 ).footer() ).html(dt_selector.cells( rows, 6, { page: 'current' } ).data().sum().toFixed({{$general_setting->decimal}}));
 $( dt_selector.column( 7 ).footer() ).html((dt_selector.cells( rows, 5, { page: 'current' } ).data().sum()-dt_selector.cells( rows, 6, { page: 'current' } ).data().sum()).toFixed({{$general_setting->decimal}}));
        }
        else {
            $( dt_selector.column( 5 ).footer() ).html(dt_selector.column( 5, {page:'current'} ).data().sum().toFixed({{$general_setting->decimal}}));
            $( dt_selector.column( 6 ).footer() ).html(dt_selector.column( 6, {page:'current'} ).data().sum().toFixed({{$general_setting->decimal}}));
            $( dt_selector.column( 7 ).footer() ).html((dt_selector.column( 5, {page:'current'} ).data().sum()-dt_selector.column( 6, {page:'current'} ).data().sum()).toFixed({{$general_setting->decimal}}));
        }
    }
